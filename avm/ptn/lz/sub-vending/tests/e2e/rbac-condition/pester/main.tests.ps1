...
Describe 'Bicep Landing Zone (Sub) Vending Tests' {
    ...
    Context 'Role-Based Access Control Assignment Tests' {
        It 'Should Have a Role Assignment for an known AAD Group with the Network Contributor role directly upon the Subscription' {
            $iterationCount = 0
            do {
                $roleAssignment = Get-AzRoleAssignment -Scope "/subscriptions/$subscriptionId" -RoleDefinitionName 'Network Contributor' -ObjectId '896b1162-be44-4b28-888a-d01acc1b4271' -ErrorAction SilentlyContinue
