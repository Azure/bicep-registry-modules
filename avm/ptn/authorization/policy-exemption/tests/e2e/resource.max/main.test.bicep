...
metadata description = 'This module deploys a Policy Exemption at a resource scope using common parameters.'
...
param serviceShort string = 'aperesmax'
...
var storageAccountName = toLower('st${uniqueString(resourceGroupName, namePrefix, serviceShort)}')
...
module storageAccount './dependencies.bicep' = {
  scope: az.resourceGroup(subscriptionId, resourceGroupName)
  name: '${uniqueString(deployment().name, resourceLocation)}-storageAccount'
  dependsOn: [
    resourceGroup
  ]
  params: {
