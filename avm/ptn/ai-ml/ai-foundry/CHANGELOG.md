...
- Added support to provide optional `name` and `roleAssignments` per associated resource.
- Added support to specify `displayName`, `description`, and `location` for AI Foundry Project
...
- Added input parameter `lock`
- Added input parameter `includeAssociatedResources`
- Added input parameter `networking`
...
- Removed output parameter `azureVmUsername`
- Removed logic that would create a Virtual Network and associated networking resrouces. This includes a `virtual network`, `bastion`, `private DNS zones`, and `virtual machine`. All networking resources required for private networking must be provided in the `networking` input parameter.
