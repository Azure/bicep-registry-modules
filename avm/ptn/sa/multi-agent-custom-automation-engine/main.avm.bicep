...
module cosmosDb 'br/public:avm/res/document-db/database-account:0.12.0' = {
  ...
  params: {
    ...
    sqlRoleAssignmentsPrincipalIds: [userAssignedIdentity.outputs.principalId]
    sqlRoleDefinitions: [
      {
...
resource cosmosDbSqlContributorRoleDefinition 'Microsoft.DocumentDB/databaseAccounts/sqlRoleDefinitions@2021-06-15' existing = {
  name: '${cosmosDb.outputs.name}/00000000-0000-0000-0000-000000000002'
}

// #disable-next-line no-deployments-resources
// resource cosmosDbRoleAssignment 'Microsoft.Resources/deployments@2021-04-01' = {
//   name: 'cosmos-db-role-assignment'
//   properties: {
//     mode: 'Incremental'
//     template: {
//       '$schema': 'https://schema.management.azure.com/schemas/2019-04-01/deploymentTemplate.json#'
...
//             '${cosmosDbSqlContributorRoleDefinition.id}'
//           )
//           properties: {
//             principalId: userAssignedIdentity.outputs.principalId
//             roleDefinitionId: cosmosDbSqlContributorRoleDefinition.id
//             principalType: 'ServicePrincipal'
//           }
//         }
//       ]
...
module deploymentScriptCLI 'br/public:avm/res/resources/deployment-script:0.5.1' = {
  ...
  params: {
    ...
    azCliVersion: '2.69.0'
    scriptContent: 'az cosmosdb sql role assignment create --resource-group "${resourceGroup().name}" --account-name "${cosmosDb.outputs.name}" --role-definition-id "${cosmosDbSqlContributorRoleDefinition.id}" --scope "${cosmosDb.outputs.resourceId}" --principal-id "${userAssignedIdentity.outputs.principalId}"'
    managedIdentities: {
