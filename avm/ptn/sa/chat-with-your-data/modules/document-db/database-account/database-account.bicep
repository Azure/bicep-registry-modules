...
resource databaseAccount 'Microsoft.DocumentDB/databaseAccounts@2025-11-01-preview' = {
  ...
  properties: {
    ...
    capabilities: map(filter(capabilitiesToAdd ?? [], c => c != 'EnableServerless'), capability => {
      name: capability
    })
    capacityMode: contains(capabilitiesToAdd ?? [], 'EnableServerless') ? 'Serverless' : null
    minimalTlsVersion: minimumTlsVersion
    capacity: contains(capabilitiesToAdd ?? [], 'EnableServerless')
      ? null
      : {
