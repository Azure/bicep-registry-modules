...
module natGatewayPublicIp 'br/public:avm/res/network/public-ip-address:0.5.1' = if (empty(networkingConfiguration.?natGatewayResourceId ?? '') && networkingConfiguration.networkType == 'createNew') {
    ...
    privateEndpoints: [
      {
        ...
        privateDnsZoneResourceIds: !empty(networkingConfiguration.?deploymentScriptPrivateDnsZoneId ?? '')
          ? [
              networkingConfiguration.?deploymentScriptPrivateDnsZoneId ?? ''
            ]
          : [
