...
module keyVault 'modules/secrets-key-vault.bicep' =
  if (secretsKeyVault != null) {
    ...
    params: {
      ...
      keySecrets: [
        {
          secretName: secretsKeyVault.?primaryKeySecretName ?? 'Key1-Key'
          secretValue: storageAccount.listKeys().keys[0].value
        }
        {
          secretName: secretsKeyVault.?primaryConnectionStringSecretName ?? 'Key1-ConnectionString'
        ...
        {
          secretName: secretsKeyVault.?secondaryKeySecretName ?? 'Key2-Key'
          secretValue: storageAccount.listKeys().keys[1].value
        }
        {
          secretName: secretsKeyVault.?secondaryConnectionStringKeySecretName ?? 'Key1-ConnectionString'
          secretValue: 'DefaultEndpointsProtocol=https;AccountName=${storageAccount.name};AccountKey=${storageAccount.listKeys().keys[1].value};EndpointSuffix=core.windows.net'
        }
      ]
    }
  }
...
type secretsKeyVaultType = {
  ...
  @description('Optional. Default to Key1-Key. The primary key secret name to create.')
  ...
  primaryConnectionStringSecretName: string?
  ...
  secondaryConnectionStringKeySecretName: string?
}
