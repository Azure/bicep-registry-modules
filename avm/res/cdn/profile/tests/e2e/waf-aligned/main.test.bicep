...
module testDeployment '../../../main.bicep' = [
  for iteration in ['init', 'idem']: {
    ...
    params: {
      ...
      originGroups: [
        {
          name: 'dep-${namePrefix}-waf-primary-origin-group'
          loadBalancingSettings: {
            additionalLatencyInMilliseconds: 50 // WAF: Performance - Low latency threshold
            sampleSize: 4 // WAF: Reliability - Multiple samples for accuracy
            successfulSamplesRequired: 3 // WAF: Reliability - Majority consensus
          }
          healthProbeSettings: {
            probePath: '/health' // WAF: Reliability - Health monitoring
            probeProtocol: 'Https' // WAF: Security - Encrypted health checks
            probeRequestType: 'GET' // WAF: Performance - Lightweight checks
            probeIntervalInSeconds: 30 // WAF: Reliability - Frequent monitoring
          }
          sessionAffinityState: 'Disabled' // WAF: Performance - Distribute load evenly
          trafficRestorationTimeToHealedOrNewEndpointsInMinutes: 5 // WAF: Reliability - Quick recovery
          origins: [
            {
              name: 'dep-${namePrefix}-waf-primary-origin'
              hostName: '${nestedDependencies.outputs.storageAccountName}.blob.${environment().suffixes.storage}'
              httpPort: 80
              httpsPort: 443
              priority: 1 // WAF: Reliability - Primary origin
              weight: 1000 // WAF: Performance - Full traffic weight
              enabledState: 'Enabled'
              enforceCertificateNameCheck: true // WAF: Security - Certificate validation
            }
            {
              name: 'dep-${namePrefix}-waf-secondary-origin'
              hostName: '${nestedDependencies.outputs.secondaryStorageAccountName}.blob.${environment().suffixes.storage}'
              httpPort: 80
              httpsPort: 443
              priority: 2 // WAF: Reliability - Failover origin
              weight: 1 // WAF: Reliability - Standby mode
              enabledState: 'Enabled'
              enforceCertificateNameCheck: true // WAF: Security - Certificate validation
            }
          ]
      ...
      afdEndpoints: [
        {
          ...
          routes: [
            {
              name: 'dep-${namePrefix}-waf-web-route'
              originGroupName: 'dep-${namePrefix}-waf-primary-origin-group'
              customDomainNames: [
                'dep-${namePrefix}-waf-primary-${serviceShort}-domain'
              ]
              enabledState: 'Enabled'
              forwardingProtocol: 'HttpsOnly' // WAF: Security - HTTPS only
              httpsRedirect: 'Enabled' // WAF: Security - Force HTTPS
              linkToDefaultDomain: 'Enabled'
              patternsToMatch: ['/*']
              supportedProtocols: ['Https'] // WAF: Security - HTTPS only
              cacheConfiguration: {
                queryStringCachingBehavior: 'IncludeSpecifiedQueryStrings' // WAF: Performance - Selective caching
                queryParameters: 'version,lang' // WAF: Performance - Cache by version and language
                compressionSettings: {
                  ...
                  isCompressionEnabled: true // WAF: Performance - Enable compression
                }
              }
              ruleSets: [
                {
