...
resource avmTelemetry 'Microsoft.Resources/deployments@2023-07-01' = if (enableTelemetry) {
  name: take('46d3xbcp.res.desktopvirtualization-workspace.${replace('-..--..-', '.', '-')}.${substring(uniqueString(deployment().name, location), 0, 4)}', 64)
  properties: {
    mode: 'Incremental'
    template: {
...
module workspace_privateEndpoints 'br/public:avm-res-network-privateendpoint:0.4.0' = [for (privateEndpoint, index) in (privateEndpoints ?? []): {
