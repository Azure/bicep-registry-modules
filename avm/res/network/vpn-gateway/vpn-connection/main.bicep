...
param ipsecPolicies resourceInput<'Microsoft.Network/vpnGateways/vpnConnections@2024-07-01'>.properties.ipsecPolicies = []

@description('Optional. The traffic selector policies to be considered by this connection.')
param trafficSelectorPolicies resourceInput<'Microsoft.Network/vpnGateways/vpnConnections@2024-07-01'>.properties.trafficSelectorPolicies = []

@description('Optional. List of all VPN site link connections to the gateway.')
param vpnLinkConnections resourceInput<'Microsoft.Network/vpnGateways/vpnConnections@2024-07-01'>.properties.vpnLinkConnections = []

@description('Optional. Routing configuration indicating the associated and propagated route tables for this connection.')
param routingConfiguration resourceInput<'Microsoft.Network/vpnGateways/vpnConnections@2024-07-01'>.properties.routingConfiguration?
...
resource avmTelemetry 'Microsoft.Resources/deployments@2025-04-01' = if (enableTelemetry) {
  name: '46d3xbcp.res.network-vpngateway-vpnconnection.${replace('-..--..-', '.', '-')}.${substring(uniqueString(deployment().name), 0, 4)}'
  properties: {
    mode: 'Incremental'
    template: {
