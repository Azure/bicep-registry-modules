...
module keyVault 'secrets-key-vault/main.bicep' = if (secretsKeyVaultReference != null) {
  ...
  params: {
    keyVaultName: secretsKeyVaultReference!.keyVaultName

    keySecrets: [
      {
        secretName: secretsKeyVaultReference.?primaryWriteKeySecretName ?? 'Primary-Write-Key'
        secretValue: databaseAccount.listKeys().primaryMasterKey
      }
      {
        secretName: secretsKeyVaultReference.?primaryReadOnlyKeySecretName ?? 'Primary-Readonly-Key'
        secretValue: databaseAccount.listKeys().primaryReadonlyMasterKey
      }
      {
        secretName: secretsKeyVaultReference.?primaryWriteConnectionStringSecretName ?? 'Primary-Write-ConnectionString'
        secretValue: databaseAccount.listConnectionStrings().connectionStrings[0].connectionString
      }
      {
        secretName: secretsKeyVaultReference.?primaryReadonlyConnectionStringSecretName ?? 'Primary-Readonly-ConnectionString'
        secretValue: databaseAccount.listConnectionStrings().connectionStrings[2].connectionString
      }
      {
        secretName: secretsKeyVaultReference.?secondaryWriteKeySecretName ?? 'Secondary-Write-Key'
        secretValue: databaseAccount.listKeys().secondaryMasterKey
      }
      {
        secretName: secretsKeyVaultReference.?secondaryReadonlyKeySecretName ?? 'Secondary-Readonly-Key'
        secretValue: databaseAccount.listKeys().secondaryReadonlyMasterKey
      }
      {
        secretName: secretsKeyVaultReference.?secondaryWriteConnectionStringSecretName ?? 'Secondary-Write-ConnectionString'
        secretValue: databaseAccount.listConnectionStrings().connectionStrings[1].connectionString
      }
      {
        secretName: secretsKeyVaultReference.?secondaryReadonlyConnectionStringSecretName ?? 'Secondary-Readonly-ConnectionString'
