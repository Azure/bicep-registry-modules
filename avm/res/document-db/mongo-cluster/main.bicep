...
module mongoCluster_privateEndpoints 'br/public:avm/res/network/private-endpoint:0.10.1' = [
  for (privateEndpoint, index) in (privateEndpoints ?? []): {
    name: '${uniqueString(deployment().name, location)}-mongoCluster-PrivateEndpoint-${index}'
    scope: resourceGroup(
...
@export()
@description('The type for the firewall set.')
...
@export()
@description('The type for the network ACLs.')
