...
Describe 'Module tests' -Tag 'Module' {
  ...
  Context 'Custom User-defined-types tests' -Tag 'UDT' {
    ...
    foreach ($moduleFolderPath in $moduleFolderPaths) {
      ...
      It "[<moduleFolderName>] Each UDT property description should start with a one word category starting with a capital letter, followed by a dot, a space and the actual description text ending with a dot." -TestCases $deploymentFolderTestCases {
      ...
      It "[<moduleFolderName>] Conditional UDT property descriptions should contain a 'Required if' followed by the condition making the parameter required." -TestCases $deploymentFolderTestCases {
...
Describe 'Test file tests' -Tag 'TestTemplate' {
  ...
  Context 'General test file' {
    ...
    It '[<moduleFolderName>] Bicep test deployment files in a [defaults] folder should have a parameter [serviceShort] with a value ending with [min]' -TestCases ($deploymentTestFileTestCases | Where-Object { $_.testFilePath -match '.*[\\|\/]defaults[\\|\/].*' }) {
    ...
    It '[<moduleFolderName>] Bicep test deployment files in a [max] folder should have a [serviceShort] parameter with a value ending with  [max]' -TestCases ($deploymentTestFileTestCases | Where-Object { $_.testFilePath -match '.*[\\|\/]max[\\|\/].*' }) {
    ...
    It '[<moduleFolderName>] Bicep test deployment files in a [waf-aligned] folder should have a [serviceShort] parameter with a value ending with [waf]' -TestCases ($deploymentTestFileTestCases | Where-Object { $_.testFilePath -match '.*[\\|\/]waf\-aligned[\\|\/].*' }) {
