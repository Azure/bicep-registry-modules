...
Describe 'Test ReadMe generation' {
    ...
    BeforeEach {
        ...
        Mock Invoke-WebRequest { return @{
                Content = (Get-Content -Path (Join-Path $repoRootPath 'utilities' 'tests' 'pipelines' 'src' 'telemetry.md'))
                Headers = @{
    ...
    It '[Set-AVMModule] Runtime test' {

        . (Join-Path $repoRootPath 'utilities' 'tools' 'Set-AVMModule.ps1')

        Mock Get-CrossReferencedModuleList { return ((Get-Content -Path 'C:\Users\alsehr\OneDrive - Microsoft\Docs\Code\temp\SetAVMModuleCache\crossReferencesBackup.json') | ConvertFrom-Json -AsHashtable) }
        ...
        $cStartTime = Get-Date
        Set-avmModule -ModuleFolderPath 'C:\dev\ip\bicep-registry-modules\Upstream-Azure\avm\res\compute\availability-set' -SkipBuild -Recurse -SkipVersionCheck #-Async
