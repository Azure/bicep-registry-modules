...
"""
    ...
    if (-not $SkipReadMe) {
        ...
        $ReadMeScriptFilePath = (Join-Path (Get-Item $PSScriptRoot).Parent.FullName 'pipelines' 'sharedScripts' 'Set-ModuleReadMeClassic.ps1')
    } else {
        ...
        try {
            $job = $relevantTemplatePaths | ForEach-Object -ThrottleLimit $ThrottleLimit -AsJob -Parallel {
                ...
                if (-not $using:SkipReadMe) {
                    ...
                    Set-ModuleReadMeClassic @readmeInputObject
                }
            }
