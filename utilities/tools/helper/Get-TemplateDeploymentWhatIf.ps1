...
.EXAMPLE
Get-TemplateDeploymentWhatIf -TemplateFilePath 'C:/key-vault/vault/main.bicep' -ParameterFilePath 'C:/key-vault/vault/.test/parameters.json' -DeploymentMetadataLocation 'WestEurope' -ResourceGroupName 'aLegendaryRg'
...
.EXAMPLE
Get-TemplateDeploymentWhatIf -TemplateFilePath 'C:/resources/resource-group/main.json' -ParameterFilePath 'C:/resources/resource-group/.test/parameters.json' -DeploymentMetadataLocation 'WestEurope'
        ...
        switch ($deploymentScope) {
            ...
            'subscription' {
                ...
                if ($PSCmdlet.ShouldProcess('Subscription level deployment', 'WhatIf')) {
                    $res = New-AzDeployment @DeploymentInputs -Location $DeploymentMetadataLocation -WhatIf
                }
                break
            }
            'managementGroup' {
                if ($PSCmdlet.ShouldProcess('Management group level deployment', 'WhatIf')) {
                    $res = New-AzManagementGroupDeployment @DeploymentInputs -Location $DeploymentMetadataLocation -ManagementGroupId $ManagementGroupId -WhatIf
                }
                break
            }
            'tenant' {
                ...
                if ($PSCmdlet.ShouldProcess('Tenant level deployment', 'WhatIf')) {
                    $res = New-AzTenantDeployment @DeploymentInputs -Location $DeploymentMetadataLocation -WhatIf
                }
                break
            }
            default {
