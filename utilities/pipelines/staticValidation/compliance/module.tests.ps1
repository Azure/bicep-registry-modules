    ...
    Context 'General test file' {
        ...
        It '[<moduleFolderName>] `sericeShort` should be unique across repository' -TestCases $deploymentTestFileTestCases {
            ...
            if (($testFileContent | Out-String) -match "param serviceShort string = '(.*)'") {
                ...
                $otherTestFiles = Get-ChildItem -Path $repoRootPath -Recurse -Include 'main.test.bicep' | Where-Object { $_.FullName -ne $testFilePath }
                foreach ($otherTestFilePath in $otherTestFiles.FullName) {
                    $otherTestFileContent = Get-Content -Path $otherTestFilePath
                    if (($otherTestFileContent | Out-String) -match "param serviceShort string = '(.*)'") {
                        $otherTestFileContent = $null # Remove from memory
                        $otherServiceShort = $Matches[1]
                        $shortendTestPath = $testFilePath -replace ('{0}[\\|\/]' -f [regex]::Escape("$repoRootPath"))
                        $shortedOtherTestPath = ($otherTestFilePath -replace ('{0}[\\|\/]' -f [regex]::Escape("$repoRootPath")), '')
